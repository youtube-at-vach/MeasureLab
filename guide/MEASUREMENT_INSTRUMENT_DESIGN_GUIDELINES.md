# MeasureLab GUI Design Guideline v2
...
| 項目 | 内容 |
| --- | --- |
| 文書名 | MeasureLab GUI Design Guideline v2 |
| ステータス | Normative |
...
- **必須:** クリッピング、I/O バッファエラー、処理失敗、校正不成立、条件不整合を結果と関連付ける。
...
| 状態の種類 | 配置 | 例 |
| --- | --- | --- |
| アプリ全体 | MainWindow の共通領域 | AudioEngine 状態、I/O バッファエラー、グローバル出力先 |
| モジュール全体で持続 | タブに隠れないモジュール領域 | 出力中、取得中、クリッピング、手順失敗 |
...
- 瞬間的なクリッピングや I/O バッファエラーは、その試行の履歴としてラッチする。
...
| Live Analyzer | ライブ波形・スペクトル・指標 | 取得状態、チャンネル、クリッピング、Hold | Run / Stop、レンジ、トリガ、主要解析条件 | 描画と取得の分離、参照スナップショット |
