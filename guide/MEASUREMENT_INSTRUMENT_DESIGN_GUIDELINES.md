...
| Detachable Wrapper | 分離、分割、撮影、ログ | 共通機能を再実装せず、能力宣言へ従う |
...
各モジュールに同じボタンを手作業で追加するのではなく、共通ラッパー、能力宣言、共有コントロール、テーマ、翻訳基盤を用いる。一方、コンパクトと 2 窓分割は、用途に合うモジュールだけが明示的に宣言する。
...
- プロットが存在するという理由だけで、分割やコンパクト表示を必須にしない。
...
比較やエクスポートに使うデータでは軸・校正情報を保持し、個別測定の再現に必要な追加条件を失わない設計にする。
