...
デジタルオーディオの世界では、信号の大きさは通常 **dBFS (Decibels relative to Full Scale)** という相対値で表されます。これはデジタル上の最大値（天井）を 0 dBFS とする単位ですが、これだけでは現実世界の物理的な大きさ（電圧や音圧）がわかりません。使用する機材やゲイン設定によって、0 dBFS が示す実際の物理量は変化するためです。

キャリブレーションとは、「デジタルの 0 dBFS が、現実世界では何ボルト（または何パスカル）に相当するか」という対応関係を測定し、ソフトウェアに設定する作業です。これを行うことで、波形を実際の「電圧（V）」や「音圧（dB SPL）」で直読できるようになります。
