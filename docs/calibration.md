...
入力感度が未校正の場合、オシロスコープの振幅はデジタルフルスケールに対する相対値 `FS` で表示されます。物理電圧との対応が確定していない状態では `V` を表示しません。入力感度を校正すると、測定値とカーソルが電圧表示へ切り替わります。
...
* **dBu**: 0.775 Vrms を基準 (0 dBu) とした電圧の単位です。 ($20 \log_{10}(V / 0.775)$) 業務用音響機器で標準的に使用される基準です。
* **dB SPL**: 音圧レベル (Sound Pressure Level) です。基準となる極めて小さな音圧 ($20 \mu Pa$) を 0 dB SPL とする単位で、マイク入力の校正 (SPL校正) を行うことで使用可能になります。
...
* **入力・出力電圧校正**:
    * **電圧計 (テスター)**: **TrueRMS (真の実効値)** 対応のものが最適です。安価なテスターは平均値から正弦波を仮定して計算するため、複雑な波形では誤差が生じます。TrueRMS対応テスターは波形に関わらず実効値を正確に測定できます。
