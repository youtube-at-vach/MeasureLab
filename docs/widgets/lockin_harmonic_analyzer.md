...
* **Harmonic Compensation (高調波補正)**: このチェックボックスをオンにすると、現在保持されている補正データ（振幅と位相）に基づいて、出力信号に逆相の歪み成分を重畳し、系全体の歪みを低減します。
* **Update Compensation (補正値更新)**: 現在の測定結果を「系の歪み」としてキャプチャし、補正テーブルを更新します。
    * **手順**: ループバック状態で本ボタンを押し、THDが最小化されるまで数回繰り返すことで、極めてクリーンなテスト信号を生成できます。
* **Clear Compensation (補正値クリア)**: 現在の補正テーブルをリセットし、純粋な正弦波出力に戻します。

### 3. Input Routing (入力ルーティング) タブ
