...
* **Harmonics & THD**: Displays the distortion components (2nd to 5th harmonics) and Total Harmonic Distortion (THD) generated by the device for each frequency (in the "Harmonics & THD" tab).
    * 💡 **What is THD?**: When a pure single tone (sine wave) is input into a device, it's the percentage of "unwanted sounds at integer multiples of the original frequency (harmonics)" that are spontaneously generated inside the device. The lower this value, the more "faithfully it is outputting the input sound exactly as it is (true to the original sound)."
