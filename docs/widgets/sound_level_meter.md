...
音の大きさは「dB (デシベル)」で表されますが、デシベル自体は割合（倍率）を表す対数（掛け算のスケール）です。
騒音計で測る「dB SPL」は、人間が聞き取れる最小の音圧を「0 dB」とし、そこから何倍の圧力になっているかを表します。

例えば、40 dB SPL と 70 dB SPL は、数値の差は30ですが、音圧エネルギーとしては1000倍の違いがあります。
人間の耳は非常に広い範囲の音圧を聞き取れるため、数値を扱いやすくするために対数スケールが使用されています。
