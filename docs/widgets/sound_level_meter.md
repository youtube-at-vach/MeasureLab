...
Detachable Wrapperでウィンドウを分離している際、「Compact」ボタンを押すとコンパクトモードになります。SPL値とLeq値などの主要な数値のみが画面いっぱいに大きく表示され、騒音計（専用ハードウェア）のような視認性になります。なお、未キャリブレーション時の警告表示はコンパクトモードでは非表示になります。
