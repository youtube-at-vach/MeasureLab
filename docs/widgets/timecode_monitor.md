...
* **Gen Mode (生成モード)**:
    ...
    * **JAM**: JAMボタンで取り込んだ外部タイムコード（または選択したメモリスロット）に同期して出力します。

* **JAM Mem**:
    `Gen Mode` を `JAM` に設定した際に、どのメモリスロット（Mem 1〜5）の値を使用して出力するかを選択します。
...
JAM ボタンで取り込んだ外部タイムコードの履歴を確認できます。
最大5つのスロット（Mem 1〜5）に、キャプチャされた時間（Captured）と、現在そのスロットが保持している時間（Current）が表示されます。

* **Slot**: メモリスロット番号 (1-5)。
* **Captured**: そのスロットにキャプチャされた瞬間のタイムコード値。
* **Current**: そのスロットの現在の値（Free Runなどで進行している場合）。
