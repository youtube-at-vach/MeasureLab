...
* **Stop Freq (終了周波数)**
    * 分析の終了周波数 (Hz) を指定します。
* **Spacing (間隔)**
    * スペクトルを計算する周波数間隔を `Log` (対数)、`Lin` (リニア)、`Integer` (整数)、`Int x Sync` (整数 × サンプル同期)、各種 `Octave` (`1/3 Octave` 等) から選びます。グラフのX軸表示もこれに連動して切り替わります。
...
* **Mains Power (電源ノイズ)**: 基本の電源周波数 (50/60 Hz または両方) と、指定した次数までの高調波ターゲット周波数を生成します。
* **Musical Scale (平均律音階)**: 設定可能なA4の基準周波数(デフォルト 440 Hz)に基づき、標準的な音階 (C0 から G9) に対応するターゲット周波数を生成します。
