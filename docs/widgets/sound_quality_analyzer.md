...
従来の測定器は電圧や周波数といった物理量を計測しますが、それだけでは人間が「うるさい」「不快だ」と感じる感覚を完全には説明できません。
心理音響学（Psychoacoustics）は、人間の聴覚メカニズムや脳の認知特性をモデル化し、音の「粗さ（Roughness）」や「鋭さ（Sharpness）」といった主観的な感覚を客観的な数値として評価するアプローチです。
