...
「普通の歪み率測定では良い数値が出るのに、聴くと音が悪い」といった現象の原因を探るのに適しています。

## 操作方法

### 測定の開始と停止
1.  **Measurement Mode** で測定したい項目（MIM, SPDR, PIM）を選びます。
...
### 1. Measurement Mode (測定モード)
このツールの核となる機能です。3つのモードから選択します。

*   **MIM (Multitone Intermodulation Distortion)**
    *   **概要**: 30個以上の音を同時に鳴らす「マルチトーン」信号を使います。音楽信号に近い複雑な負荷をかけたときの「歪み＋ノイズ (TD+N)」を測定します。
...
*   **SPDR (Spurious Free Dynamic Range)**
    *   **概要**: 1kHzの信号を出した時に、「信号以外の余計な成分（スプリアス）」が信号よりどれくらい低い位置にあるかを測ります。
...
*   **PIM (Passive Intermodulation / 2-Tone)**
    ...
    *   **設定**: **Freq 1 / Freq 2** で2つの周波数を指定します。
...
### 1. アンプの実力チェック (MIM測定)
「スペック表の歪み率は良いのに、激しい曲を聴くと音が濁る」ような場合、MIM測定が有効です。
...
1.  **Mode** を `PIM` にします。
2.  **Freq 1** を `18000` (18kHz)、**Freq 2** を `19000` (19kHz) など、高い周波数に設定してみます。
