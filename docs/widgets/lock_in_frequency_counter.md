...
## ☕ コーヒーブレイク：なぜ「ロックイン」が必要なの？

普通の周波数カウンター（例えばチューナー）は、「1秒間に波が何回やってきたか」を数えます。しかし、これでは「1000.0001回」のような微小なズレを測るには、なんと1万秒（約3時間）も待って波の数を数え続ける必要があります。
そこで「ロックイン」の出番です！
これは、入力信号とピッタリ同じペースで走る「仮想の伴走者（NCO）」を作り、2人が並走しながら「今、何ミリ差がついたか（位相差）」を測る方法です。この方法なら、わずか0.1秒走っただけでも「あ、少し遅れたな」と超高精度にズレを見抜くことができるのです。
...
このタブでは、測定された周波数偏差や時間間隔の統計的な分布をヒストグラムとして可視化します。

* **Distribution View**: **Frequency Histogram** (周波数の分布, Hz) と **Time Interval Histogram** (周期の分布, 秒) を切り替えます。
* **Bins**: ヒストグラムのビン（細かさ）の数を調整します。
* **Clear Distribution**: 収集された分布データをリセットします。
* **Distribution Statistics**: 選択した分布モードの包括的な統計情報（平均、標準偏差、Peak-to-Peak、RMS Jitter、アラン偏差、歪度、尖度、最小値、最大値）を表示します。
