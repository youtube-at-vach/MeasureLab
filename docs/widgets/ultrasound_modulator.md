...
## ☕ コーヒーブレイク：音を「超音波のロケット」に乗せる

人間には聞こえない「超音波」。実は、この超音波に普通の音（声や音楽）を乗せて飛ばすと、空中で音が復元されて「特定の場所だけに音が聞こえる（パラメトリックスピーカー）」という不思議な現象が起こります。
このウィジェットは、あなたの声を「超音波のロケット（搬送波）」に乗せるためのツール（変調器）です。これを使えば、普通のPCとオーディオインターフェースを使って、最先端の「音響レーザー」の実験ができちゃいます！
