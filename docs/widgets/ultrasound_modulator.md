...
Ultrasound Modulator（超音波モジュレーター）は、可聴域のオーディオ信号を振幅変調（AM）し、超音波リファレンススピーカー（40kHz中心）などで再生可能な信号に変換するためのウィジットです。
...
* **安全機能**: 出力ゲインが危険なレベルに達した場合、警告を表示します。
...
#### Advanced Options
* **Enable √ Pre-distortion**: 振幅変調の復調時に生じる歪みを補正するため、信号にあらかじめ平方根処理（$\sqrt{1+m(t)}$）を適用します。パラメトリックスピーカー等で、自己復調時の歪みを低減したい場合に有効です。
