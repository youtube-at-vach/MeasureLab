...
Detachable Wrapperの分離ウィンドウ時に「Compact」ボタンを押すことで、ヒートマップ表示のみを最大化するコンパクトモードに対応しています。

## ☕ コーヒーブレイク：音の指紋「声紋（せいもん）」ってなに？

人間の声や鳥の鳴き声などをこのグラフで見ると、まるで指紋のように「その音特有の模様」が浮かび上がります。これを「声紋」と呼びます。

例えば、「あ」と「い」と声に出してみてください。喉から出ている元の音は同じでも、口の形が違うため、響きやすい高さ（周波数）が変わります。スペクトログラムで見ると、この違いが「色の縞模様（フォルマント）」としてハッキリと現れます。
一人ひとりの声帯の大きさや骨格の違いもこの模様に現れるため、警察の科学捜査で犯人の声を特定するのにも、これと全く同じ原理が使われています。見えない「声」の正体が、鮮やかな模様になって現れる瞬間はとても面白いですよ！
...
- **FFT Size (周波数分解能)**
    ...
    - ※ 「Speed」設定が速い場合、リアルタイム性能を維持するために選択可能な最大サイズが自動的に制限されます。

- **Window (窓関数)**
    - ノイズ分析には `hann` (標準) や `blackman` が適しています。

- **Scale (スケール)**
    - 縦軸（周波数）の表示スケールを切り替えます。
    - **Linear**: 定規のように等間隔な目盛りです。高音の細かい倍音を見るのに向いています。
    - **Log**: 対数目盛り。ピアノの鍵盤のように、低い音域が広く表示されます。音楽的な音程を見たいときに最適です。
    - **Mel**: メル尺度。人間の耳が「2倍高い」と感じる幅を等間隔にした、ちょっと不思議な目盛りです。声の分析によく使われます。

- **Colormap (配色)**
    - グラフの色使いを変更します。
    - **viridis / plasma / inferno / magma / cividis**: 科学的によく使われる、明るさの変化が均等で見やすい配色です。
    - **turbo**: 虹色のようなカラフルな配色で、細かいレベル差を見分けるのに適しています。

- **Speed (流れる速さ)**
    - グラフがスクロールするスピードを調整します。
    - **Fast (Realtime)**: リアルタイムで流れます。短時間の変化を見るのに向いています。
    - **Medium / Slow / Meteor**: ゆっくり流れます。環境音のモニタリングなど、長い時間をかけて変化を観察する場合に使います（数分〜10分間の履歴を一画面に表示）。これらの低速モードでは、間引きによる見逃しを防ぐため、期間内の最大値を保持する「Max Hold」処理が行われます。
