...
2. Routingを開き、図中の測定入力L/Rへの割当を確認します。
...
モニター出力は、配線後の測定入力L/Rを常に試聴します。ソースの選択は不要です。
例えば測定入力L＝Wet、R＝Dry参照なら、左に処理後、右に参照信号が聴こえます。
無音に割り当てたチャンネルはモニターも無音です。DUT未ロード時も、仮想ループバックの測定入力を試聴できます。

モニター音量は−60～0 dBです。音量調整、float32変換、振幅制限はモニター分岐にだけ適用し、測定値を変更しません。
物理デバイスがステレオの場合は先頭2チャンネル、モノの場合は左右の平均を出力します。
モニターでは追加の出力チャンネル選択やディザを適用せず、設定済みの測定入力をコピーします。
...
解析の出力先カードに「↓ モニター出力」を1つ表示し、測定入力L/Rをまとめて試聴することと状態を示します。
...
- デバイス変更はモニターOFF時に行います。ON/OFFと音量は測定中も変更でき、測定ストリームやDUTを再起動しません。
...
- DUTエラー時は測定入力を無音化し、モニターも停止します。プラグインを再ロードしてから、モニターを再度有効にしてください。
