...
アプリが突然動かなくなったり、おかしな動作をしたりしたとき、画面を見ただけでは「なぜそうなったのか」が分かりません。
ログは、飛行機の「フライトレコーダー（ブラックボックス）」のようなものです。
「12:00:01 にオーディオデバイスを開こうとした」「12:00:02 にUSBケーブルが抜かれたからエラーになった」というように、アプリ自身が裏でブツブツと呟いている独り言がすべて記録されています。
トラブルが起きたときは、この「独り言」を読んであげることで、すぐに原因を突き止めることができるのです！
