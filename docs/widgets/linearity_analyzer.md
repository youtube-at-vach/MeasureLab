...
## ☕ コーヒーブレイク：直線性（リニアリティ）って何？

「直線性」という言葉は難しく聞こえますが、要するに「入れた分だけ、きっちり比例して出てくるか？」ということです。
例えば、体重計に1kgの重りを乗せたら「1kg」と表示され、100kgの重りを乗せたら「100kg」と表示されるのが「直線性が良い」状態です。
しかし、デジタルオーディオ機器では「大きな音は得意だけど、とても小さな音になると誤差が出てしまう（体重計が1gの羽をうまく測れないように）」ということが起こります。
このウィジェットは、機器が「どれくらい小さな音まで正確に測れるか」をチェックする、いわば「超精密な体重計の検査ツール」なのです！
