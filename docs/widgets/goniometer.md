...
このウィジェットは **Split Window**（ウィンドウ分割）機能に対応しています。Detachable Wrapper のヘッダーにある「Split Window」ボタンを押すことで、リサージュ図形表示部と設定コントロール部を別々のウィンドウに切り離して配置することができます。

## コンパクトモード (Compact Mode)

Detachable Wrapperの分離ウィンドウ時に「Compact」ボタンを押すことで、リサージュ図形のみを最大化するコンパクトモードに対応しています。メーターとして常に表示しておきたい場合に最適です。

## リサージュ図形とは

ゴニオメーターの画面に表示される波形は「リサージュ図形」と呼ばれるものです。
左チャンネルの信号をY軸（縦）、右チャンネルの信号をX軸（横）として同時に描画することで、ステレオ信号の広がりや位相差を視覚的に表現しています。

完全に同じ音が左右から入力されると直線になり、左右の信号に位相差や音量の違いが生じるにつれて、図形は円や複雑な曲線へと変化します。
