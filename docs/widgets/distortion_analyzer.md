...
## ☕ コーヒーブレイク：オーディオにおける歪み

アンプやDACなどの機器に純粋なサイン波（正弦波）を入力した際、出力される信号に元の周波数以外の成分（倍音やノイズ）が混入することがあります。これを「歪み（ひずみ）」と呼びます。
Distortion Analyzerは、元の信号波形が機器の増幅過程でどれだけ変形したかを定量的に測定するツールです。
