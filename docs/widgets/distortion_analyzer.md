...
**Note**: 周波数スイープは単一トーン（正弦波）による THD+N 解析です。振幅スイープでは、正弦波の THD+N に加えて SMPTE、DIN、CCIF の IMD 測定を選択できます。
...
* **設定**: Start（開始音量）と End（終了音量）、および `THD+N (Sine)`、`SMPTE IMD`、`DIN IMD`、`CCIF IMD` の測定方式を選択します。
* **Sweep Results**: 結果はグラフにプロットされます。Y軸の単位は `dB` または `Percent (%)` から選択でき、パーセント表示時は自動的におおよそ対数スケールで表示されます。
* 無効になった測定点はグラフおよび比較データから除外されます。

#### Stability (安定性ログ)

長時間のウォームアップや安定性を確認するため、1秒ごとに周波数、入力レベル、利得、THD、THD+N、ノイズ、SINAD、測定有効性を記録します。dB指標と周波数は左右のY軸で確認でき、CSVへ保存できます。

温度センサーを使用していないため、この結果だけで変化を温度ドリフトと断定することはできません。
...
* **Signal Generator**:
    ...
    * **SMPTE / DIN / CCIF**: IMD測定用の2トーン信号です。CCIFでは19 kHz/20 kHz付近を使用するため、インターフェース帯域とナイキスト余裕の警告を確認してください。
    * **AES17 Dynamic Range (-60dBFS)**: -60 dBFSの997 HzトーンとAES17 20 kHzフィルターでダイナミックレンジを測定します。`Run Guided AES17` は0 dBFS校正レベルの検証、クリップ確認、セトリング、測定、レポート作成を順番に実行します。
...
* **Amplitude**: 信号の強さです。以下の単位を選択できます。
    ...
    * **dBu**: 0.775Vrms基準。
    * **Vrms**: 電圧実効値。
    * 出力校正がない場合、物理単位は選択できず、安全な `dBFS` 表示へ戻ります。
...
測定画面上部には入力・出力チャンネル、`CAL` / `UNCAL`、サンプルレート、FFTサイズ、フィルター、平均回数が常に表示されます。

入力クリップ、XRUN、フレーム欠落、非有限値、出力フルスケール超過を検出すると、その測定ランは `INVALID` としてラッチされます。問題を解消して新しい測定を開始するまで数値は有効値として表示・保存されません。
