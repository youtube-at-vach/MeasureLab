...
オーディオ機器に純粋な信号（サイン波など）を入力した際、機器内部の増幅回路などの影響で、元の信号には存在しなかった「倍音（高調波）」や「ノイズ」が付け加わってしまうことがあります。これが「歪み」です。
Distortion Analyzerは、元の純粋な波形からどれだけ余分な成分が混ざって形が崩れてしまったかを、パーセンテージ（THD）やデシベル（SINAD）で精密に数値化します。これにより、機器の出力の正確さを客観的に評価することができます。
