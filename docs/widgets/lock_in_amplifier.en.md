...
## ☕ Coffee Break: Extracting Target Signals from Noise

A lock-in amplifier is a powerful tool for measuring minute signals buried in noise.
It works by using a "reference signal" and extracting only the components of the input signal that share the exact same frequency and phase, using multiplication (synchronous detection) and a low-pass filter.
Since random noise is unrelated to the reference signal, it gets averaged out to near zero by the filter. This allows the instrument to accurately isolate and measure the target signal even in extremely noisy environments.
