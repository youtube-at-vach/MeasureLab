...
このウィジェットは **Split Window**（ウィンドウ分割）機能に対応しています。Detachable Wrapper のヘッダーにある「Split Window」ボタンを押すことで、波形ディスプレイ部と設定コントロール部を別々のウィンドウに切り離して配置することができます。
...
波形を止めて見やすくするための機能です。
特定の電圧レベル（Level）を通過した瞬間に画面を更新することで、高速な波形を静止して表示させます。
...
たまにしか起きない異常なノイズや、波形の微妙なブレ（ジッター）を、光の重なりとして視覚的に発見しやすくなります。
