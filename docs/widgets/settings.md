...
## ☕ コーヒーブレイク：バッファサイズとディザリング

* **バッファサイズ**: 一度に処理するオーディオデータ量です。大きいほど遅延は増えますが、音切れを防ぐため測定用途では安定性が向上します。
* **ディザリング**: ビット深度を下げる際に微小なノイズを加える処理です。量子化誤差が規則的な歪みになるのを防ぎ、ダイナミックレンジを維持します。
