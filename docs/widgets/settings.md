...
* **Host API**: オーディオバックエンド（API）を選択します（例: MME, WASAPI, ASIO, ALSA, JACKなど）。特定のAPIのデバイスのみをリストに表示するために使用します。
...
* **Virtual / Offline Mode**: 仮想オーディオドライバを有効にします。物理的なオーディオインターフェースがない場合に使用します。オーディオファイルを読み込み、入力デバイスからの信号のように扱うことができます。
* **Simulation Rate**: 仮想モード時のサンプリングレートを設定します。この設定は仮想モードが有効な場合のみ表示されます。
...
プロファイルには、保存時に使用していたデバイス名とホストAPIも記録されます。
