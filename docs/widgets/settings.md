...
入力、出力、SPL、周波数、1PPS、ロックインの校正値を一つの状態として保存・呼び出しできます。プロファイルを選択すると即時適用され、その後の校正変更は使用中のプロファイルへ自動保存されます。

* **名前なしの現在のキャリブレーション**: 現在値を名前付きプロファイルへ割り当てずに使用します。
* **新規**: 現在の入出力デバイスに対して、安全な未校正状態のプロファイルを作成して使用します。
* **複製**: 現在の校正値を新しい名前のプロファイルへコピーして使用します。
* **名前を変更**: 校正値とデバイス情報を変えずに、使用中のプロファイル名だけを変更します。
* **削除**: 確認後に使用中のプロファイルを削除します。現在の校正値は名前なしの状態として保持されます。

選択欄の下には、登録された入力・出力デバイスが表示されます。現在のオーディオデバイスと異なる場合は警告が表示されますが、プロファイルを呼び出してもオーディオデバイス自体は変更されません。
