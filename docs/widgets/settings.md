...
* **Host API**: オーディオインターフェースの動作モデルを選択します（Windows では ASIO, WASAPI, DirectSound など、Linux では ALSA, JACK など）。選択した API に応じてデバイスリストがフィルタリングされます。
* **Input Device**: 測定用マイクなどを接続した入力デバイスを選択します。
* **Output Device**: スピーカーなどを接続した出力デバイスを選択します。
* **Refresh Devices**: デバイスリストと Host API のリストを更新します。
...
* **Sample Rate**: サンプリング周波数を選択します。
    * 選択したデバイスでサポートされるサンプリングレートが自動的にリストアップされます。
    * サポートされているレートのみがリストに表示され、直接入力はできません。サンプリングレートの自動判定（プローブ）はバックグラウンドで行われるため、起動を妨げることはありません。
    * 高精度な測定には可能な限り高いサンプリングレート（192kHz以上など）を推奨します。
...
現在の校正設定（Input Sensitivity, Output Gain, SPL Offsetなど）をプロファイルとして名前を付けて保存できます。
異なるオーディオインターフェースやマイクを使い分ける場合に、設定を素早く切り替えるのに便利です。
プロファイルには、保存時に使用していたデバイス名も記録されます。
