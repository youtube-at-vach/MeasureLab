...
一般的なオーディオ測定では単一の正弦波を使用しますが、実際の音楽には多数の周波数が同時に含まれています。
複数の周波数が同時にアンプを通過すると、信号同士が干渉し合い、本来存在しない不要な周波数（混変調歪）が発生することがあります。
意図的に複数の信号を同時に入力することで、実際の音楽再生に近い負荷条件下での性能を評価することができます。
