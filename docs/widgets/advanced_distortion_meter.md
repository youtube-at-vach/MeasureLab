...
## ☕ コーヒーブレイク：なぜ「複雑な音」でテストするの？

オーディオの性能を測るのに、普通は「ピー」という一つの音（単一正弦波）を使います。でも、私たちが普段聴く音楽は、ボーカル、ギター、ドラムなど、たくさんの音が同時に鳴っていますよね。

例えるなら、**「一人で歌うのは上手い歌手（アンプ）が、合唱団に混ざると途端に周りの音につられて音程を外してしまう」** ような現象が、実際のオーディオ機器でも起こります。複数の音が同時にアンプを通ると、お互いに干渉しあって、元々なかった「濁った音（混変調歪）」を生み出してしまうのです。

この Advanced Distortion Meter は、わざと複数の音を同時に鳴らして、「合唱団の中でもちゃんと全員が正確に歌えているか（音が濁らないか）」をテストする、とても厳しい試験官なのです！
...
- **J-Test**
    - **概要**: デジタルオーディオ伝送（S/PDIFやUSBなど）におけるジッター（時間的なゆらぎ）を可視化するための特殊なテスト信号を生成します。
    - **見方**: グラフ上でFs/4の主信号の裾野の広がりや、不要なスパイク（スプリアス）がないかを確認します。
    - **設定**: 機器の対応に合わせて **Bit Depth**（16-bit または 24-bit）を選択します。
