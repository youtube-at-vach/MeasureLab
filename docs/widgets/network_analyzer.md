...
* **Harmonics & THD (高調波歪み)**: 機器で発生した歪み成分（2〜5次高調波）と全高調波歪み（THD）を周波数ごとに表示します（「Harmonics & THD」タブ）。
    * 💡 **THD（全高調波歪み）って何？**：純粋な1つの音（サイン波）を機器に入れたときに、機器の内部で勝手に生み出されてしまう「元の音の整数倍の高さの不要な音（高調波）」の割合です。この値が低いほど「入力された音をそのまま忠実に（原音に忠実に）出力できている」ことになります。
