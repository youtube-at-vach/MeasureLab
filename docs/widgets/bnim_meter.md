...
フクロウ（そして人間も）は、左右の耳に音が届くわずかな「時間差」と「音量差」を超高速で計算し、音の方向を立体的に割り出しています。音源が左右にずれると、近い耳に音が早く届き（**ITD: 両耳間時間差**）、遠い耳では頭が障害物となり音が小さくなります（**ILD: 両耳間レベル差**）。BNIMメーターはこの聴覚メカニズムをシミュレートし、音の方向を可視化します。
