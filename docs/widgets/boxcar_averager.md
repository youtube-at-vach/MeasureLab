...
## ☕ コーヒーブレイク：「平均化」でノイズが消える魔法

うるさい人混みの中で、誰かがあなたに大事な一言を囁（ささや）いていると想像してください。1回だけでは周りの雑音にかき消されて、何を言っているのか全く分かりません。
でも、その人が「同じタイミング」で「同じ言葉」を100回繰り返してくれたらどうでしょう？
周りの雑音（話し声や車の音）は毎回バラバラでデタラメ（ランダム）ですが、囁き声の形は毎回同じです。これを100回分「重ねて平均」すると、デタラメな雑音はプラスとマイナスが打ち消し合ってゼロに近づき、毎回同じ形をしている囁き声だけが、まるで魔法のようにクッキリと浮かび上がってきます！
これが Boxcar Averager（平均化処理）の仕組みです。ノイズの海に沈んだ微小な信号を救い出す、非常に強力なテクニックなのです。
...
分析範囲の限定や、外部信号との同期に関する設定です。

- **Gate (ゲート)**: 分析する時間範囲を限定する機能です。特定の反射音だけを取り出したり、処理負荷を下げたりする場合に使います。
    - **Enable**: 機能をON/OFFします。
    - **Start**: 周期の開始点（t=0）から、実際に記録を開始するまでの遅延時間を設定します。
    - **Width**: 記録する幅（長さ）を設定します。

- **External Sync (外部同期)**: Modeを `External Reference` にしたときのみ表示されます。
    - **Ref (参照チャンネル)**: トリガーとして使用するチャンネル（Left または Right）を選びます。
    - **Edge (エッジ)**: **Rising**（立ち上がり）、**Falling**（立ち下がり）、または **Free Run**（同期なしで連続キャプチャ）を選びます。
    - **Lvl (レベル)**: トリガー判定を行う電圧レベル(閾値)を設定します。

## 使用例

### アンプやエフェクターのステップ応答を見る
