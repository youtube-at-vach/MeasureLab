...
## ☕ コーヒーブレイク：「平均化」でノイズが消える仕組み

Boxcar Averagerは、繰り返し発生する信号の波形を正確に捉えるための処理（加算平均）です。
トリガー信号を基準にして、同じタイミングで波形を何度も切り出し、それらを重ね合わせて平均します。
ランダムなノイズはプラスとマイナスが入り乱れているため、平均化を繰り返すとゼロに近づいて消えていきますが、毎回同じタイミングで発生する「目的の波形」はそのまま残ります。これにより、ノイズに埋もれた微小な信号を鮮明に浮かび上がらせることができます。
