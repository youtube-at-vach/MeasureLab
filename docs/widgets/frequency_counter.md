...
「周波数（Hz：ヘルツ）」と聞くと難しそうですが、実はとてもシンプルです。これは「1秒間に何回繰り返すか」を表す言葉です。
例えば、海に立って波を見ていると想像してください。1秒間に波が1回押し寄せてきたら「1 Hz」、2回なら「2 Hz」です。心臓の鼓動（心拍数）が1分間に60回なら、1秒間に1回なので「1 Hz」になります。

音も同じで、空気が1秒間に何回震えたかで「音の高さ」が決まります。人間の声はだいたい 100 Hz ～ 1000 Hz くらい。1秒間に何百回も空気が震えているなんて不思議ですよね。
周波数カウンターは、この「目に見えない超高速な波の数」を、間違いなく正確に数えてくれる頼もしいストップウォッチのようなツールです。

## コンパクトモード (Compact Mode)

Detachable Wrapperの分離ウィンドウ時に「Compact」ボタンを押すことで、周波数や各種統計値のみを極大サイズで表示するコンパクトモードに対応しています。遠くから数値を監視したい場合に便利です。
