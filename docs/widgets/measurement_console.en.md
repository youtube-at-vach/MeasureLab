...
* **Add Instrument**: Search by a translated or English name, then select a widget to add.
  Instruments already in the console appear disabled at the end of the list. Press Enter
  after searching to add the selected instrument.
...
* **Stop All**: Stops every compatible instrument currently running in the console. It does
  ...
  Failed stops remain visible at the bottom of the console. Hover to see the instrument names,
  or click to show the first affected instrument.
...
presets are available by name in the **Layout** menu. The current preset name appears in the
toolbar, and the outlined button identifies the selected preset. Click it again to cycle
instruments through the panes; in a main
...
* **Layout → Undo Default Console**: Restore the instruments and arrangement from before
  the last default-console reset.
* **Layout → Workspaces**: Save and recall up to 12 named instrument arrangements.
...
The bottom bar mirrors the audio state, sample rate, output destination, and latched I/O
warnings from the main window. It uses the main window's existing 500 ms status update and
does not poll the audio engine a second time.

## Restoring a Workspace

The console stores its membership, dock arrangement, size, lock state, and compatible
compact-mode selections, along with the selected preset and main instrument. Changes are saved
after a short pause and when the console closes. If a saved layout cannot be restored safely,
MeasureLab falls back to a visible default layout. On small displays, the default arrangement
is adjusted to remain usable. The arrangement from the larger screen is retained for the next return to
...
the same time.
Workspace snapshots contain the screen arrangement, not measurement results or acquisition
conditions. Use each instrument's save or comparison features for measurement data.
