...
Detachable Wrapperの分離ウィンドウ時に「Compact」ボタンを押すことで、各ノイズタイプの相関スコア（パーセンテージ）のみを大きく表示するコンパクトモードに切り替わります。ノイズの性質が一目でわかるようになります。

## ☕ コーヒーブレイク：「ノイズ」の色って？

「ノイズ」と一口に言っても、原因によって音のキャラクターが全く違います。このウィジットではノイズを3つの成分（色や種類）に分けて分析します。

* **ホワイトノイズ (White Noise)**: 「サーーー」という音。テレビの砂嵐や、激しい雨の音に似ています。光のすべての色が混ざると「白」になるように、すべての高さの音が均等に混ざっているためこう呼ばれます。
* **1/fノイズ (Flicker Noise)**: 「ザーーー」という音。ホワイトノイズより少し低音が強く、ピンクノイズとも呼ばれます。滝の音や小川のせせらぎなど、自然界の音に多く含まれています。
* **ハムノイズ (Hum)**: 「ブーン」という低い音。古い冷蔵庫の音に似ています。コンセントから来る交流電気（東日本なら50Hz、西日本なら60Hz）がそのまま漏れ聞こえている状態です。

このツールを使えば、耳で聞いただけでは分かりにくい「ノイズの正体」が一目で分かり、アンプなどのノイズ退治に大活躍します！
