...
## ☕ コーヒーブレイク：ノイズから目的の信号だけを抽出する仕組み

ロックインアンプは、ノイズに埋もれた微小な信号を測定するための強力な手法です。
測定器自身が発振する「参照信号」と同じ周波数・同じ位相の成分だけを、入力信号から掛け算（同期検波）とローパスフィルターを用いて抽出します。
参照信号と無関係なランダムなノイズはフィルターによって平均化されてゼロに近づくため、非常にノイズが多い環境下でも目的の信号だけを高精度に測定することができます。
