...
The **Lock-in Harmonic Analyzer** is a module that utilizes the principles of a lock-in amplifier combined with Matrix Projection to measure Total Harmonic Distortion (THD) and THD+N with extremely low noise and high precision.

Unlike conventional FFT-based distortion measurements (which require window functions), this widget completely eliminates spectral leakage by fitting basis functions over precise integer cycles of the test signal. This enables measurements in ultra-low distortion regimes (e.g., below -160 dBc) that exceed the limitations of traditional FFT analysis.

## Unique Measurement Principle

The most significant feature of this widget is its **windowless, coherent matrix projection** analysis approach.

1. **Coherent Segment Extraction**: The module precisely interpolates the zero-crossing points (rising edges) of the input Reference signal to extract exactly an integer number of signal cycles.
2. **Basis Matrix Generation**: Based on the detected fundamental frequency $\omega$, a basis matrix $B$ is generated consisting of a DC component and the sine and cosine waves for each harmonic component ($1\omega$ through $n\omega$).
3. **Matrix Projection**: By solving a system of linear equations (normal equations) for the incoming measurement signal vector $Y$, the projection components (coefficients) onto each basis signal are calculated simultaneously.
   $$ \text{Coefficients} = (B^T B)^{-1} B^T Y $$
4. **Harmonic and Residual Separation**: The amplitude and phase of each harmonic are calculated from the obtained coefficients. By subtracting all extracted harmonic components from the original signal, the pure residual noise component is cleanly separated.

This method **fundamentally avoids the main lobe broadening and side-lobe leakage caused by window functions** required in FFT measurements. It allows for the accurate separation and extraction of minuscule distortion components from the surrounding background noise floor.
