...
* **Meas Mode**: 通常のスイープ測定と非線形モデル測定モードを切り替えます。
* **Amp Steps**: 非線形モデル測定モードでの振幅のステップ数（例：5）を設定します。
* **Real-time Display (リアルタイム表示)**: 有効にすると、測定中にリアルタイムで測定グラフをプロットして更新します。無効（デフォルト）にすると、測定中（スイープ中）は解析計算を保留してデータを蓄積し、測定終了後に一括で解析してグラフをプロットします。これにより、重い解析処理によるCPU負荷やGILの競合を防ぎ、オーディオのバッファアンダーラン（音切れ）を確実に防止します。
