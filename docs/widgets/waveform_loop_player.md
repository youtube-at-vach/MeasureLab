...
ファイルは自動正規化せず、Playback Gainで明示的にレベルを設定します。フルスケール付近のファイルではサンプル間ピークが0 dBTPを超え、サンプルレート変換後のサンプル値が0 dBFSを超える場合もあります。必要に応じて再生ゲインを下げてください。3 dBの減衰は予防策ですが、常に十分とは限りません。共通のエラー表示は量子化前のミックス出力が0 dBFSを超えると警告を保持します。警告がなくてもTrue Peakの安全性は保証されず、物理出力への自動減衰は行いません。
