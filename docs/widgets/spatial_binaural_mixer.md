...
2. 追加された各トラック行で以下の設定を行います:
   ...
   * **トラックを削除 (X) ボタン**: そのトラックを削除します。読み上げソフトでは「トラックを削除」と案内され、キーボードでも実行できます。
...
* **前／次のプレビュー区間 (◀ / ▶) ボタン**: 設定した長さ分だけ開始位置を前後に移動し、素早く別の箇所をプレビューできます。各ボタンには読み上げ用の操作名があります。
...
畳み込みとミックスの後、推定True Peakが-1 dBTPを超える場合はレンダリング全体を減衰します。ステレオのバランスは維持し、圧縮処理は行いません。4倍補間と余裕の確保でサンプル間ピークによる過大出力を抑えますが、すべてのDACの再構成フィルターで非クリップを保証するものではありません。
