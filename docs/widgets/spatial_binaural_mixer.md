...
## ☕ コーヒーブレイク：HRTF（頭部伝達関数）

私たちが音の方向（前後や上下）を認識できるのは、音源の位置によって耳たぶや頭部が音に与える影響（周波数特性や到達時間の微細な変化）が異なるためです。

この「頭や耳が音に与える変化のパターン」をデータ化したものが「HRTF（頭部伝達関数）」です。Spatial Binaural Mixerは、このHRTFデータを用いて音声信号に空間的な変化を畳み込むことで、通常のヘッドホンでも音が外の3D空間から聞こえてくるような立体音響を作り出します。
