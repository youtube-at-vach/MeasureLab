...
その秘密は、私たちの「耳たぶの形」や「頭の丸み」にあります！
音が後ろから来る時と、前から来る時では、耳たぶに反射する様子がわずかに変わり、音の「色合い（周波数特性）」や「届くタイミング」に複雑な変化が起きます。脳はこの変化を瞬時に計算して、音の場所を特定しているのです。
この「頭や耳が音に与える変化のパターン」をまるごと録音・データ化したものが「HRTF（頭部伝達関数）」です。
