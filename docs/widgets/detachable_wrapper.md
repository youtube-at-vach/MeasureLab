...
MeasureLabのウィジットは、最初は1つのメインウィンドウに収まっていますが、「Detach Window（切り離し）」ボタンを押すことで、それぞれの測定器を独立した小さなウィンドウとして画面の好きな場所に配置できます。
左の画面にはオシロスコープとシグナルジェネレータ、右の画面には巨大なスペクトラムアナライザを並べる...といった具合に、複数のモニターを使って自分だけの「実験室のコックピット」を自由に構築できるのが、この機能の最大の魅力です！
