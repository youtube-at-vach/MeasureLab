...
補償と音量合わせの後、サンプル間ピークを含む推定True Peakが-1 dBTPを超える場合に減衰します。RMS音量合わせでは補償後と元信号の両ファイルに同じ追加減衰を適用し、比較時のレベル関係を維持します。補償処理ですでに生じたクリッピングは復元できません。4倍補間による推定は、すべてのDACで非クリップを保証するものではありません。
