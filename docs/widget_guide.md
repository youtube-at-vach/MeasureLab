...
ここにはたくさんのウィジット（測定器）がありますが、それぞれどんな役割なのでしょうか？

- **オシロスコープ (Oscilloscope)** は、音の形をそのまま写し取る**「カメラ」**です。
- **スペクトラムアナライザ (Spectrum Analyzer)** は、音を周波数ごとに分ける**「プリズム」**です。（光を虹色に分けるように！）
- **歪み率計 (Distortion Analyzer)** は、信号に混ざった不純物だけを見つけ出す**「顕微鏡」**です。

これらを組み合わせることで、見えない「音」の正体をあらゆる角度から解き明かすことができます。オーケストラの指揮者のように、目的に合わせて自由にウィジットを配置してみてください！
...
## ☕ コーヒーブレイク：音を「波」で見るか、「成分」で見るか？

これから様々な解析ツールが登場しますが、大きく分けて「時間」で見るか「周波数（成分）」で見るかの2つの視点があります。
例えるなら、美味しいスープを味わうときのようなものです。
「オシロスコープ（時間領域）」は、スープを一口飲んで「最初は熱くて、だんだん味が濃くなるな」と時間の変化を味わうこと。
一方、「スペクトラムアナライザ（周波数領域）」は、スープを科学的に分析して「塩が〇グラム、胡椒が〇グラム入っているな」と材料（成分）を特定することです。
目的に合わせて、この2つの視点を使い分けるのが測定の醍醐味です！
