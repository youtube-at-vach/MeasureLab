...
このページは、MeasureLabを使用する際の注意事項（測定器としての限界）と、よくあるトラブルシューティングをまとめた付録です。
...
専用の測定器は「絶対にブレない基準」を持っていますが、MeasureLabはそのような絶対的な精度や法的な証明が求められる場面ではなく、あくまで「相対的な変化の観測」に特化しています。

### 信頼できる範囲・参考値となる範囲

本ツールが得意とするのは「相対的な変化」の観測です。

* **信頼できる用途**: 相対比較（対策前後でのノイズ低減効果の確認など）、動作確認（クリップや発振の有無）、トラブルシュート（ハムノイズやグリッチの特定）。
* **参考値にとどまるもの**: 絶対値（校正なしでの電圧や音圧の物理量）、微小領域の測定（PCやオーディオインターフェース自体の限界に依存する-100dB以下の歪みなど）。
...
* **Settings ウィジェット**: 正しいデバイス（ASIO/WASAPI等）が選択されているか確認してください。
* **OSの共有モード**: 他の録音ソフトやブラウザ、Web会議ツールがデバイスを占有していないか確認してください。

### ❓ グラフの動きがガタガタする・音が途切れる

* **バッファサイズ**: Settings ウィジェットで **Buffer Optimization** を `STABLE` または `ULTRA` に変更してください。
* **CPU負荷**: 他の重いアプリを終了してください。
* **入出力バッファ警告**: 赤い「入出力バッファエラー」が表示された測定には、サンプルの欠落や重複が含まれる可能性があります。設定やCPU負荷を改善してから警告をクリックし、解除後に測定をやり直してください。問題が続いていれば警告は再度表示されます。
