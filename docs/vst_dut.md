...
- プラグインは別プロセスのメインスレッドでロード・処理します。ロードは30秒、処理応答は1秒でタイムアウトします。
  独自パネルも同じプロセスで開き、表示中もMeasureLabの画面は操作できます。
  パネル表示中は処理応答のタイムアウトを適用せず、閉じる操作には5秒のタイムアウトを適用します。
