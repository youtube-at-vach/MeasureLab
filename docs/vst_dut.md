...
5. 配線を変える場合は「ルーティング」ボタンからRoutingへ移動し、図中の「チャンネル配線を編集…」でモノ／ステレオ、DUTの入力元、左右の測定入力への戻し先を選びます。
...
サイドバーの[Routing](widgets/routing.md)で物理出力デバイスを選び、モニター出力をONにするとDUTの音を試聴できます。
モニター操作はRoutingに集約しています。VST画面の「ルーティング」ボタンから移動できます。初期状態はOFF、音量は−20 dBです。
試聴する信号は配線後の測定入力L/Rに固定されており、ソース選択はありません。Dry参照や無音の割当も反映されます。
モニター音量や物理出力の障害は測定値を変更しません。
仮想モード専用の試聴機能で、バッファ遅延やクロック差による途切れが発生する場合があります。

### 画面例

Routingでチャンネル配線を展開した画面です。VST画面はプラグイン管理に集中し、配線・モニター設定はRoutingで操作します。

![日本語のRouting配線画面](assets/vst_dut/routing-ja.png)

この画像は表示確認用の構成例です。実デバイスや実プラグインの動作検証結果ではありません。
