...
4. 複数のプラグインを含むバンドルでは「バンドル内のプラグイン名（省略可）」にチェックを入れ、
   表示された欄に名前を入力します。「VST3をロード」を押すと、画面上部にロード状態が表示されます。
5. 「ルーティング」タブでモノ／ステレオ、DUTの入力元、左右の測定入力への戻し先を選びます。
6. 「パラメータ」タブで名前を検索・選択し、スライダーまたは数値入力で正規化値（0～1）を設定して「適用」を押します。
   スライダーの刻みは0.001、数値入力は小数点以下6桁です。編集だけではプラグインの値は変わりません。
...
日本語の配線画面と、英語・ダークテーマのパラメータ画面です。
プラグイン名・パラメータ名は、ロードしたVST3が公開する名前をそのまま表示します。

![日本語のVST3配線画面](assets/vst_dut/routing-ja.png)

![ダークテーマのパラメータ検索と値の編集](assets/vst_dut/parameters-dark.png)

[GUIレビューの比較画像と確認内容](vst_dut_gui_review.md)も参照できます。
