...
ASIO対応は実験的な機能で、既定では無効です。必要なASIOドライバーを別途導入したうえで、インストール時に「実験的なASIO対応を有効にする」を選択できます。この選択は更新時にも再利用されます。インストール先には手動切替用スクリプトも同梱されます。詳細は同フォルダーの `README_ASIO.txt` を参照してください。
