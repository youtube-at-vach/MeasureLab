...
## ☕ コーヒーブレイク：なぜスピーカーのインピーダンスを測るの？

スピーカーには「8Ω（オーム）」や「4Ω」と書かれていますが、実はこの抵抗値は常に一定ではありません。
スピーカーが音を出してコーン紙が前後に動いているとき、磁石の中でコイルが動くため、実は「小さな発電機」のように振る舞います！そのため、音の高さ（周波数）によって電気の通りにくさ（インピーダンス）が大きく変わるのです。
このインピーダンスが一番高く跳ね上がるポイント（最低共振周波数、$f_0$ と呼びます）を見つけることで、そのスピーカーが「一番気持ちよく振動できる音の高さ」が分かります。これはブランコを漕ぐときの最適なタイミングを見つけるようなもので、スピーカーの箱（エンクロージャー）を設計するための最初の大事なステップになります。
