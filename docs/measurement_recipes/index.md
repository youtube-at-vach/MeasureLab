...
## ☕ コーヒーブレイク：なぜ「レシピ」と呼ぶの？

料理を作る時、「食材」と「フライパン」だけ渡されても困ってしまいますよね。「この料理には、どの道具を使って、どの順番で火を通せばいいか」という**レシピ（手順書）**があって初めて美味しい料理が完成します。

オーディオ測定も全く同じです。MeasureLabにはたくさんの強力なツール（ウィジット）が用意されていますが、それらを「どう組み合わせるか」が重要です。このページでは、測定のプロのノウハウを、誰でも真似できる「レシピ」としてまとめています。今日はどのメニューを作ってみましょうか？
