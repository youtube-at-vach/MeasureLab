...
Linux（apt/emerge環境）および macOS 向けに、OSパッケージのインストールから仮想環境（`.venv`）の構築、必要なPython依存パッケージやNode.jsなどの開発ツールのインストールまでを自動化する便利なスクリプトが用意されています。
...
Linux でサポート対象外のパッケージマネージャ環境や管理者権限が使えない環境では、スクリプトは OS パッケージの自動導入をスキップし、必要パッケージの案内を表示した上で venv/pip セットアップを続行します。
