...
def run_challenge():
    ...
    json_path = '/Users/vach/MeasureLab/hammerstein_kernel_sample2.json'
    ...
    training_amps = np.linspace(0.01, 0.3, 10)
    ...
    reg_lambda = 1e-2  # Slightly larger regularization to avoid overfitting
    
    for fi in range(N_fft_half):
        if not passband[fi]:
            # Outside active band, simply set coefficients to zero
            W_filters[:, fi] = 0.0
            continue
            
        # Build matrix A of shape (10, 5)
        A = np.zeros((len(training_amps), 5), dtype=complex)
        for p_idx, p in enumerate([1, 2, 3, 4, 5]):
            A[:, p_idx] = Y_powers_fft[p][:, fi]
            
        b = X_train_fft[:, fi]
        
        # Check energy to prevent singular equations
        energy = np.sum(np.abs(A)**2)
        if energy < 1e-10:
            # Fallback to linear inverse
            W_filters[0, fi] = Q1_inv[fi]
            W_filters[1:, fi] = 0.0
        else:
            AH_A = np.conj(A.T) @ A
            AH_b = np.conj(A.T) @ b
            W_bin = np.linalg.solve(AH_A + reg_lambda * np.eye(5), AH_b)
            W_filters[:, fi] = W_bin
    ...
    def apply_ila_predistortion(u_val):
        ...
        for p_idx, p in enumerate([1, 2, 3, 4, 5]):
            up_val = u_val**p
            W_p = W_filters[p_idx]
            x_pred += np.fft.irfft(np.fft.rfft(up_val) * W_p, n=len(u_val))
    ...
    for amp in test_amplitudes:
        ...
        if converged_cm and not np.isnan(x_cm).any():
            ...
            cm_reductions.append(red_cm)
        else:
            cm_reductions.append(0.0)
            
        # Method B: ILA
        x_ila = apply_ila_predistortion(u_amp_filt)
        if not np.isnan(x_ila).any():
            ...
            # Limit minimum display value to 0.0 for cleaner plotting
            ila_reductions.append(max(0.0, red_ila))
        else:
            ila_reductions.append(0.0)
            
        print(f"Amp {amp:.2f} | CM: {'OK' if converged_cm else 'FAIL'} ({cm_reductions[-1]:.2f} dB) | ILA: {ila_reductions[-1]:.2f} dB")
