...
def run_closed_loop_training():
    ...
    for iteration in range(pre_train_iter):
        ...
        print(f"Pre-train Iter {iteration+1:2d}/{pre_train_iter} | Total Error: {total_err_db_pt:6.2f} dB | THD: {thd_db_pt:6.2f} dB (1 sweep)")
    ...
    for iteration in range(max_iter_fine):
        ...
        for search_step in range(max_search_steps):
            ...
            if total_err_db_cand < best_total_err_db:
                ...
                success = True
                print(f"Fine-tune Iter {iteration+1:2d}/{max_iter_fine} | Step {search_step+1} Accepted | Total Error: {best_total_err_db:6.2f} dB | THD: {best_thd_db:6.2f} dB")
                break
            else:
        ...
        if not success:
            print(f"Fine-tune Iter {iteration+1:2d}/{max_iter_fine} | Line search failed to improve error. Stopping optimization.")
            break
    ...
    plt.plot(range(0, len(history_err_db)), history_err_db[:, 0], 'o-', color='#1f77b4', linewidth=2.5, label='Total Error')
    plt.plot(range(0, len(history_err_db)), history_err_db[:, 1], 's--', color='#ff7f0e', linewidth=2.5, label='THD Error')
    ...
    plt.xlim(200, 10000)
    plt.ylim(-110, -20)
    plt.title(f"Validation: Two-Tone Spectrum (SDR Imp: {res_2tone['improvement']:.1f}dB)", fontsize=12, fontweight='bold')
    plt.xlabel('Frequency (Hz)', fontsize=10)
    plt.ylabel('Magnitude (dBFS)', fontsize=10)
    plt.grid(True, which='both', linestyle='--', alpha=0.5)
    plt.legend()
    ...
    plt.xlim(100, 15000)
    plt.ylim(-110, -20)
    plt.title(f"Validation: Multi-Tone Spectrum (SDR Imp: {res_multi['improvement']:.1f}dB)", fontsize=12, fontweight='bold')
    plt.xlabel('Frequency (Hz)', fontsize=10)
    plt.ylabel('Magnitude (dBFS)', fontsize=10)
    plt.grid(True, which='both', linestyle='--', alpha=0.5)
    plt.legend()
