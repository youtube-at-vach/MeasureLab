...
このプロジェクトでは `markdownlint-cli2` を使用してドキュメントの品質を管理しています。
以下のルールが自動的にチェック（および一部自動修正）されます。

- **空白・改行**:
    - 行末の不要な空白は削除する (MD009)
    - 見出し (H1, H2...) の前後には空行を入れる (MD022)
    - コードブロック、リストの前後には空行を入れる (MD031, MD032)
- **リスト**:
    - ファイル内でリストマーカー（`-` または `*`）を統一する (MD004)
    - 番号付きリストは `1.` から順に振る (MD029)
    - リストマーカーの後のスペースは1つにする (MD030)
- **その他**:
