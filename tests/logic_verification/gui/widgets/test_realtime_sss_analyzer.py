...
def test_realtime_sss_analyzer_averaging_freq_update(qtbot, mock_audio_engine):
    ...
    # Feed block 0
    analyzer.measurement_queue.append((0, 0, 100.0, np.zeros(5, dtype=complex)))
    widget.update_plots()
    assert "100.0 Hz" in widget.lbl_current_freq.text()

    # Feed final block of first sweep
    last_block_idx = widget.max_blocks - 1
    analyzer.measurement_queue.append((last_block_idx, 0, 20000.0, np.zeros(5, dtype=complex)))
    widget.update_plots()
    ...
    analyzer.measurement_queue.append((0, 1, 150.0, np.zeros(5, dtype=complex)))
    widget.update_plots()
