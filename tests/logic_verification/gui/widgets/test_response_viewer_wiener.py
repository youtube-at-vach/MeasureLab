...
def test_wiener_conversion_math(qtbot, mock_audio_engine, dummy_model_data):
    ...
    sigma_linear * (h_time[1] + 3 * sigma_sq * h_time[3] + 15 * (sigma_sq**2) * h_time[5])
    (sigma_linear**2) * (h_time[2] + 6 * sigma_sq * h_time[4])
    (sigma_linear**3) * (h_time[3] + 10 * sigma_sq * h_time[5])
    (sigma_linear**4) * h_time[4]
