...
def test_settings_widget_structure(qtbot):
    ...
    with patch("src.gui.widgets.settings.get_manager") as mock_get_man:
        ...
        with patch.object(SettingsWidget, "_get_current_host_api_index", return_value=0):
            ...
            assert hasattr(widget, "new_prof_btn")
            assert isinstance(widget.new_prof_btn, QPushButton)
            assert hasattr(widget, "duplicate_prof_btn")
            assert isinstance(widget.duplicate_prof_btn, QPushButton)
            assert hasattr(widget, "rename_prof_btn")
            assert isinstance(widget.rename_prof_btn, QPushButton)
            assert hasattr(widget, "del_prof_btn")
            assert isinstance(widget.del_prof_btn, QPushButton)
