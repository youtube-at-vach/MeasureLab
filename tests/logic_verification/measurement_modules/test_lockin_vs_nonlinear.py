...
def test_lockin_vs_nonlinear_consistency():
    ...
    a = {1: 1.0, 2: 0.08, 3: 0.12, 4: 0.04, 5: 0.06}
    delays = {
        1: 5.0,  # samples
        2: 8.0,  # samples
        3: 12.0,  # samples
    ...
    for p in range(1, 6):
        ...
        print(
            f"[{h_key}] Theory Phase={theory_phase:.2f}°, Meas Phase={meas_phase:.2f}°, Diff={meas_phase - theory_phase:.2f}°"
        )
    ...
    for n in range(1, 6):
        ...
        print(
            f"DEBUG: n={n}, raw_phase={meas_phase_raw_deg:.2f}, fund_phase={fund_phase_lockin_deg:.2f}, rel_phase={meas_rel_phase_deg:.2f}"
        )
        ...
        print(
            f"[{h_key}] Amplitude: Pred={pred_amp_dbfs:.2f} dBFS, Meas={meas_amp_dbfs:.2f} dBFS, Diff={amp_diff:.2f} dB"
        )
        ...
        print(
            f"[{h_key}] Rel Phase: Pred={pred_rel_phase_deg:.2f}°, Meas={meas_rel_phase_deg:.2f}°, Diff={phase_diff:.2f}°"
        )
