...
def benchmark():
    ...
    print(
        f"{'Size':<10} | {'NumPy (ms)':<12} | {'pyfftw (iface) (ms)':<20} | {'pyfftw (builder) (ms)':<20} | {'pyfftw (obj) (ms)':<20}"
    )
    print("-" * 90)
    ...
    for N in SIZES:
        ...
        t_pyfftw_iface = timeit.timeit(
            lambda input_data=input_data: pyfftw.interfaces.numpy_fft.rfft(input_data, threads=n_threads),
            number=ITERATIONS,
        )
        ...
        t_pyfftw_builder = timeit.timeit(
            lambda fft_object_builder=fft_object_builder: fft_object_builder(), number=ITERATIONS
        )
        ...
        fft_object = pyfftw.FFTW(
            aligned_in, aligned_out, direction="FFTW_FORWARD", flags=("FFTW_MEASURE",), threads=n_threads
        )
        ...
        print(
            f"{N:<10} | {avg_numpy:<12.4f} | {avg_pyfftw_iface:<20.4f} | {avg_pyfftw_builder:<20.4f} | {avg_pyfftw_obj:<20.4f}"
        )
