...
def generate_simulation_data(amplitudes, freqs, use_filter=True, snr_db=None):
    """
...
def estimate_power_kernels(amplitudes, raw_responses, freqs, max_harmonic=5):
    ...
    # Map raw kernels to frequency-mapped kernels H_p(f)
    H_mapped_list = []
    for p in range(P):
        H_raw = H_freqs[p]
        f_lookups = freqs / (p + 1)
        ...
        mag_mapped = np.interp(f_lookups, freqs, mags, left=np.nan, right=np.nan)
        phase_mapped = np.interp(f_lookups, freqs, phases, left=np.nan, right=np.nan)
        
        H_mapped = mag_mapped * np.exp(1j * phase_mapped)
        H_mapped_list.append(H_mapped)
        
    return H_mapped_list
...
def run_simulation_scenario(use_filter=True, snr_db=None, name="Scenario"):
    print(f"\n==================================================")
    ...
    raw_responses = generate_simulation_data(amplitudes, freqs, use_filter=use_filter, snr_db=snr_db)
    ...
    Y_true = generate_simulation_data([A_in], np.array([f0]), use_filter=use_filter, snr_db=None)[0, 0, :]
    ...
    # 1. Magnitude Plot
    axes[0].plot(f_plot, 20*np.log10(np.abs(H_true_plot) + 1e-12), 'k-', label='True $H(f)$', linewidth=2)
    # Proposed synthesized H(f) (only up to 100 kHz)
    ...
    # 2. Phase Plot
    axes[1].plot(f_plot, np.degrees(np.unwrap(np.angle(H_true_plot))), 'k-', label='True $H(f)$', linewidth=2)
    ...
    # Save Plot
    artifacts_dir = "/Users/vach/.gemini/antigravity/brain/f49b222f-24c4-428a-931f-8064c2ce2227"
...
def main():
    ...
    report_path = "/Users/vach/.gemini/antigravity/brain/f49b222f-24c4-428a-931f-8064c2ce2227/simulation_report.json"
    summary_report = {
        "ideal_flat": {
