...
* **Documentation Development (Astro Starlight / Node.js 24 LTS)**:
    ...
    Run `npm run check` to validate the web build, links, images, and fragments. Run `npm run build:all` to generate the web site and both language PDFs from the same content. PDF generation requires Chromium and a CJK font (`fonts-noto-cjk` on Linux).
