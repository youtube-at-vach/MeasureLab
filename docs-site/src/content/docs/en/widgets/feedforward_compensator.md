...
By loading a Hammerstein model (1st to 5th order impulse response kernels) measured and extracted by modules such as the [Nonlinear Analyzer](https://youtube-at-vach.github.io/MeasureLab/en/widgets/nonlinear_analyzer/), it generates a compensated signal with inverse distortion to actively cancel (reduce) the distortion of physical audio devices.
...
* **Load Forward Model JSON... Button:** Loads a Hammerstein model JSON file (containing kernels `h1` through `h5`) exported from the [Nonlinear Analyzer](https://youtube-at-vach.github.io/MeasureLab/en/widgets/nonlinear_analyzer/).
