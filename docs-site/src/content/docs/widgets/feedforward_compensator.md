...
[Nonlinear Analyzer](https://youtube-at-vach.github.io/MeasureLab/widgets/nonlinear_analyzer/) などで測定・抽出した機器 of Hammerstein モデル（1次〜5次のインパルス応答カーネル）をロードし、逆歪みを付加した補正信号を生成することで、実機の歪みを能動的にキャンセル（低減）します。
...
* **Load Forward Model JSON... ボタン:** [Nonlinear Analyzer](https://youtube-at-vach.github.io/MeasureLab/widgets/nonlinear_analyzer/) でエクスポートした Hammerstein モデルの JSON ファイル（`h1`〜`h5`のカーネルを含む）を読み込みます。
