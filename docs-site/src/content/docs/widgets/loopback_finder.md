---
title: "Loopback Finder (ループバック検出)"
---
...
* **Signal Level**: 検出された信号のレベル（dBFS）です。明確に検出された経路にはチェックマーク（✓）が表示されます。クリッピングにより無効な測定となった場合は感嘆符（!）が表示され、信号が検出されなかった場合はダッシュ（—）が表示されます。値が大きいほど、その経路でテストトーンがはっきり見つかったことを意味します。
...
* **内部リークの発見**: 本来繋がっていないはずのチャンネル間で信号が漏れているか（クロストークや内部ループがないか）を探せます。
