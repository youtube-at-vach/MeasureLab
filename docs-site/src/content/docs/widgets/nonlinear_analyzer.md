...
Nonlinear Analyzer（非線形アナライザー）は、オーディオ機器やアンプ等の非線形特性を詳細に測定するための高度なウィジェットです。
...
* **モデルのキャッシュとエクスポート:** 測定完了後、抽出されたモデルは自動的にアクティブモデルのキャッシュに保存され、他のモジュール（例: [Response Viewer](https://youtube-at-vach.github.io/MeasureLab/widgets/response_viewer/)）で詳細な解析やシミュレーションを行えます。また、JSONファイルとしてローカルへエクスポートすることも可能です。
...
測定された Parallel Hammerstein モデルを用いたより詳細な解析（THDマップ、ゲインコンプレッション、トーンシミュレータ、Wiener表現への変換など）を行うには、[Response Viewer (応答ビューア)](https://youtube-at-vach.github.io/MeasureLab/widgets/response_viewer/) モジュールを使用してください。
