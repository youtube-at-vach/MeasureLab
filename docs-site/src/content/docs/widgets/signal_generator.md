...
* **Transition Time (ms)**: 出力の開始・停止、ルーティング変更、再生中のパラメータ変更にRaised-Cosineフェードを適用します。既定値は10 msです。MLS、Golay、PRBS、外部トリガー測定などでサンプル列を無加工のまま出力する場合は0 msに設定します。
