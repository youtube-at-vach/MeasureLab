---
title: "Spectrogram (スペクトログラム・声紋分析)"
---
...
- **Start / Stop ボタン**: 測定の開始と停止を切り替えます。
    - スタートすると、グラフの原点（向きに応じて上端または右端）から新しいデータが現れ、古いデータは反対側へ流れていきます（ウォーターフォール表示）。

### グラフの見方

- **時間軸 (Time)**: 時間の経過を表します。向きに応じて、現在時刻が右端（左へ流れる）または上端（下へ流れる）になります。
- **周波数軸 (Frequency)**: 周波数（音の高さ）を表します。値が大きいほど高い音です。
...
- **Direction (向き)**
    - スペクトログラムの表示向き（ウォーターフォールの流れる方向）を選択します。
    - **X: Time**: 横軸を時間、縦軸を周波数として表示します。
    - **X: Frequency**: 横軸を周波数、縦軸を時間として下方向へ流れるように表示します。
- **Colormap (配色)**
    - グラフの色使いを変更します。
...
3. ノイズが鳴っている時の画面を観察します。
    - **時間軸に沿って伸びる直線**: ファンやモーター、電源ハムなど、定常的なノイズです。周波数軸を見て周波数を特定できます。
    - **周波数軸を横断する線**: 衝撃音やクリックノイズです。
