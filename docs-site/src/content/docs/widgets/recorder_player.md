...
独立ウィンドウへ分離した後は **Compact** を使用できます。Compact では、ファイル名、再生／録音状態、経過時間、シーク、Loop、Sync、未保存状態、入出力モード、再生ゲインを確認できます。Load と詳細設定は通常表示へ戻して操作します。Compact の切り替えや通常表示への復帰によって、進行中の再生や録音は停止しません。
...
入力された信号を一時ファイルへストリーミング記録し、指定したファイルとして書き出します。長時間録音でも、録音データ全体をメモリへ保持しません。

* **Record / Stop Recording**: 録音の開始と停止を行います。
* **Save Recording**: 録音したデータをファイルに保存します。録音停止中のみ押すことができます。保存処理はバックグラウンドで行われ、進捗ダイアログが表示されます。大きなファイルでも画面がフリーズすることなく保存できます。
    * 録音停止後は **未保存** と表示されます。未保存のまま新しい録音を始める場合、保存、破棄、キャンセルを選択します。
* **Input Mode (入力モード)**
    * **Stereo**: L/R両方を録音します。
    * **Left / Right**: 指定した片方のチャンネルのみを録音します。

* **Recorded Info**: 現在録音されている長さと、録音中、未保存、保存中、保存済みなどの状態を表示します。
...
3. **Play** を押して再生を開始します。
4. 別のウィジェット（Spectrum Analyzerなど）を開き、入力信号が正しく来ているか確認します。
