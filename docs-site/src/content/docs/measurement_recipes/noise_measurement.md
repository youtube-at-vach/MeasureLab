...
| 目的 | 推奨ツール | 概要 |
| :--- | :--- | :--- |
| **ノイズのスペクトルが見たい** | [Spectrum Analyzer](https://youtube-at-vach.github.io/MeasureLab/widgets/spectrum_analyzer/) | ノイズがどの周波数にどれくらい含まれているか（分布）を確認します。最も一般的な測定です。 |
| **ノイズの種類を分析したい** | [Noise Profiler](https://youtube-at-vach.github.io/MeasureLab/widgets/noise_profiler/) | 「サー（ホワイトノイズ）」「ブーン（ハム）」といった成分に分解し、それぞれの寄与度を数値化します。 |
| **ノイズの時系列変化が見たい** | [Raw Time Series](https://youtube-at-vach.github.io/MeasureLab/widgets/raw_time_series/) | 「時々鳴るプチノイズ」や「DCオフセットのふらつき」など、時間の経過に伴う変化を監視します。 |
| **まれな異常ノイズを数えたい** | [Event Detector](https://youtube-at-vach.github.io/MeasureLab/widgets/event_detector/) | ポップコーンノイズやRTNなどの発生回数と発生レートを測定します。 |
...
通常の `Spectrum` モードは特定の周波数のレベルを測るものですが、ノイズのように様々な周波数に分布する信号の測定には適していません。
`PSD` モードは「1Hzあたりのパワー密度（V/√Hz など）」を示すものであり、帯域幅に依存せずにノイズフロアを正確に評価できます。
...
JFETやオペアンプなどで時々発生するポップコーンノイズやRTNを、一定時間内のイベント回数として比較します。

### 測定手順

1. **Raw Time Series**または**Oscilloscope**で通常ノイズと異常イベントの振幅を確認します。
2. **Event Detector**を起動し、対象チャンネルを選択します。
3. 通常ノイズでは超えず、異常イベントが超えるように**Threshold**を設定します。
4. 必要に応じて**Polarity**、**Hysteresis**、**Holdoff**を設定します。
5. **Start**を押し、部品ごとに同じ時間だけ測定します。
6. **Event Count**と**Event Rate**を比較します。

### 比較時の注意点

* 入力ゲイン、サンプルレート、Threshold、Hysteresis、Holdoff、測定時間をすべて同じにします。
* DUTの温度やウォームアップ時間も揃えると、再現性が向上します。
* `CLIPPING`または`I/O BUFFER ERROR`が表示された測定結果は比較に使用しないでください。
...
* **見逃し**: ウィジェットが表示されていない間（バックグラウンドなど）の挙動は環境設定によりますが、基本的には描画は継続されます。ただし、非常に短いスパイクノイズは、画面の描画更新間隔の間に落ちてしまう可能性があります（エイリアシング）。確実に捉えたい場合はオシロスコープウィジェットのトリガー機能の併用も検討してください。
