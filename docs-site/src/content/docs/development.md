...
* **ドキュメント開発（Astro Starlight / Node.js 24 LTS）**:
    ...
    Web、リンク、画像、フラグメントをまとめて検証するには `npm run check`、Webと日英PDFを同じコンテンツから生成するには `npm run build:all` を実行します。PDF生成にはChromiumとCJKフォント（Linuxでは`fonts-noto-cjk`）が必要です。
