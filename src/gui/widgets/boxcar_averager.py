...
class BoxcarAverager(MeasurementModule):
    ...
    def _callback(self, indata, outdata, frames, time, status):
        ...
        if write_idx + frames <= buf_len:
            ...
            self.sample_index_ring[write_idx : write_idx + frames] = np.arange(
                abs_start, abs_start + frames, dtype=np.int64
            )
        else:
            # Wrap around
    ...
    def process(self):
        ...
        # Process Data
        if "Internal" in self.mode:
            self._process_internal_mode(data, idxs)
        else:
            self._process_external_mode(data, idxs)

    def _process_internal_mode(self, data: np.ndarray, idxs: np.ndarray) -> None:
        """
        Process the buffer in internal synchronous folding mode.
        """
    ...
    def _process_external_mode(self, data: np.ndarray, idxs: np.ndarray) -> None:
        """
        Process the buffer in external trigger (reference sync) mode.
        """
...
class BoxcarAveragerWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.gate_start_spin.setValue(
            float(self.module.gate_start_samples) / float(self.module.audio_engine.sample_rate) * 1000.0
        )
        ...
        self.gate_width_spin.setValue(
            float(self.module.gate_length_samples) / float(self.module.audio_engine.sample_rate) * 1000.0
        )
