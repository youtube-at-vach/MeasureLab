...
from src.core.wiener_estimator_core import (
...
class WienerEstimatorSignals(QObject):
...
class WienerEstimatorWorker(QThread):
    ...
    def run(self):
        try:
            ...
            logger.error("WienerEstimatorWorker Error: %s", e, exc_info=True)
...
class WienerEstimator(MeasurementModule):
    def __init__(self, audio_engine: AudioEngine):
        self.audio_engine = audio_engine
        self.signals = WienerEstimatorSignals()
    ...
    @property
    def name(self) -> str:
        return "Wiener Estimator"

    @property
    def description(self) -> str:
        return "Estimates linear block (LTI) and static nonlinearity (SNL) of a Wiener model."

    def get_widget(self):
        return WienerEstimatorWidget(self)
    ...
    def start_measurement(self):
        ...
        self.worker = WienerEstimatorWorker(self)
        self.worker.start()
    ...
    def _execute_measurement(self, worker):
        ...
        for avg in range(self.averages):
            ...
            if getattr(self.audio_engine, "offline_mode", False):
                # Simulated system
                sim = SimulatedWienerSystem(sample_rate)
...
class WienerEstimatorWidget(QWidget):
    def __init__(self, module: WienerEstimator):
        super().__init__()
        self.module = module
