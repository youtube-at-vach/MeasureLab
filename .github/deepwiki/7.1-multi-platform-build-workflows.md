...
graph TD
    ...
    subgraph "Distribution Assets"
        WIN_SETUP["MeasureLab Setup.exe (installer)"]
        WIN_DIR["MeasureLab/ (onedir ZIP + ASIO scripts)"]
...
The Windows build generates a directory-based PyInstaller bundle with ASIO support, packages it as the standard Inno Setup installer, and also publishes the folder-based ZIP build.
...
| :--- | :--- | :--- |
| `installer` | Standard Windows distribution | Inno Setup packages the `onedir` output |
| `onedir` | Folder-based ZIP and installer input | `--windowed --name MeasureLab` |
